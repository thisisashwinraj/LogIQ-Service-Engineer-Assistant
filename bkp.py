...
def set_cache_model_number(brand, sub_category, model_number):
    ...
    print(st.session_state.cache_model_number)
...
def build_context_cache(input_brand, input_sub_category, input_model_number):
    ...
    service_guide_title, path_to_service_guide = service_guide_db.fetch_guides_by_model_number(input_model_number)
    ...
    if selected_menu_item == "LogIQ Chatbot":
        with st.sidebar:
            ...
            if not st.session_state.cache_model_number:
                ...
                input_sub_category = st.selectbox("Select Appliance", available_categories, index=None, placeholder="Select Appliance", label_visibility="collapsed")
                    
                available_brands = appliances_db.fetch_brands_by_sub_category(input_sub_category)
                input_brand = st.selectbox("Select the Brand", available_brands, index=None, placeholder="Select the Brand", label_visibility="collapsed")
                        
                available_models = appliances_db.fetch_models_by_brand_and_sub_category(input_brand, input_sub_category)
                input_model_number = st.selectbox("Select the Model", available_models, index=None, placeholder="Select the Model", label_visibility="collapsed")

                if st.button("🛠️ Configure Chatbot", use_container_width=True, disabled=not(input_model_number)):
                    set_cache_model_number(input_brand, input_sub_category, input_model_number)
                    st.rerun()

                st.markdown("<BR>"*7, unsafe_allow_html=True)
                with st.popover("Acknowledgement", use_container_width=True):
                    st.success("Google Cloud credits are provided for this project! #AISprint")
        ...
        if st.session_state.cache_model_number:
            ...
            if "gemini_flash" not in st.session_state:
                st.toast(f"Configuring chatbot for {st.session_state.cache_model_number}. This may take a minute.")
                ...
                with preloader_work_in_progress:
                    ...
                    """
                    warning_configuring_chatbot = st.sidebar.warning(configuration_information, icon="🛠️")

                    try:
                        st.session_state.gemini_flash = build_context_cache(st.session_state.cache_brand, st.session_state.cache_sub_category, st.session_state.cache_model_number)
                        st.session_state.flag_use_context_cache = True

                    except Exception as error:
                        service_guide_db = ServiceGuides()
                        service_guide_title, path_to_service_guide = service_guide_db.fetch_guides_by_model_number(st.session_state.cache_model_number)
                        
                        service_engineer_chatbot = ServiceEngineerChatbot()
                        st.session_state.service_guide = service_engineer_chatbot.post_service_guide_to_gemini(service_guide_title, path_to_service_guide)

                        st.session_state.gemini_flash = service_engineer_chatbot.construct_flash_model(st.session_state.cache_brand, st.session_state.cache_sub_category, st.session_state.cache_model_number)
            ...
            if st.session_state.cache_model_number:
                ...
                if st.sidebar.button("⚙️ Clear and Reconfigure", use_container_width=True):
            ...
            if "chat" not in st.session_state:
                st.session_state.chat = st.session_state.gemini_flash.start_chat(history=st.session_state.messages)
                ...
                end_time = time.time()
                print(f"... built cache in {end_time-start_time} sec")
            ...
            if prompt := st.chat_input("Type your question here..."):
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Thinking... (Please wait 15-20 seconds for the chatbot to respond to your query.)"):
                        ...
                        try:
                            ...
                            fallback_message = f"Sorry, I am unable to answer this.\nReason: {err}"
            ...
            end_time = time.time()
            print(f"response generated in {end_time-start_time}sec ")

        else:
            st.markdown("<BR><BR><H2>LogIQ - Service Engineer Chatbot ⛑️</H2>", unsafe_allow_html=True)
            st.markdown("<H4>Diagnose, Troubleshoot, and Repair White Label Appliances with Expert Precision!</H4>Facing an issue with your kitchen appliance, but not sure whom to call? Or perhaps you need immediate assistance with a repair? Simply describe your problem, and our chatbot powered by Gemini 1.5 will help you troubleshoot these problems in a single tap!", unsafe_allow_html=True)
